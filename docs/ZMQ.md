# The Current/Future Status of ZMQ in Incognito

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Incognito
...
matching supported by both Incognito and ZMQ. The `format`, `context` and `event`
...
The Incognito daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Incognito daemon also ensures that
...
The Incognito daemon will send a `txpool_add` pub exactly once for each
