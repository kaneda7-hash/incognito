...
# a main offline incognitod, does most of the tests
# a restricted RPC incognitod setup with RPC payment
# two local online incognitods connected to each other
N_INCOGNITODS = 5

# 4 wallets connected to the main offline incognitod
# 1 wallet connected to the first local online incognitod
# 1 offline wallet
N_WALLETS = 7
...
incognitod_base = [builddir + "/bin/incognitod", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--p2p-bind-port", "incognitod_p2p_port", "--rpc-bind-port", "incognitod_rpc_port", "--zmq-rpc-bind-port", "incognitod_zmq_port", "--zmq-pub", "incognitod_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "incognitod_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
...
wallet_base = [builddir + "/bin/incognito-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_INCOGNITODS):
  command_lines.append([str(18180+i) if x == "incognitod_rpc_port" else str(18280+i) if x == "incognitod_p2p_port" else str(18380+i) if x == "incognitod_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "incognitod_zmq_pub" else builddir + "/functional-tests-directory/incognitod" + str(i) if x == "incognitod_data_dir" else x for x in incognitod_base])
  if i < len(incognitod_extra):
    command_lines[-1] += incognitod_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/incognitod' + str(i) + '.log', 'a+'))
