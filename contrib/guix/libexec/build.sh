...
# Copyright (c) 2022-2024 The Incognito Project
...
DISTNAME="incognito-${HOST}-${VERSION}"
...
# target, so we don't spend time building Incognito binaries.
...
GIT_ARCHIVE="${DIST_ARCHIVE_BASE}/incognito-source-${VERSION}.tar.gz"
...
if [ ! -e "$GIT_ARCHIVE" ]; then
    ...
    | sort \
    | tar --create --transform "s,^,incognito-source-${VERSION}/," --mode='u+rw,go+r-w,a+X' --files-from=- \
    | gzip -9n > ${GIT_ARCHIVE}
...
(
    ...
    # Setup the directory where our Incognito build for HOST will be
