...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/incognitod"
  then
    incognitod="$dir/incognitod"
    break
  fi
done
if test -z "$incognitod"
then
  echo "incognitod not found"
  exit 1
fi
echo "Found: $incognitod"

TORDIR="$DIR/incognito-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting incognitod"
  tail -n 400 "$HOME/.incognito/incognito.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Incognito\ \'" | tail -n 20
  exit 1
fi
